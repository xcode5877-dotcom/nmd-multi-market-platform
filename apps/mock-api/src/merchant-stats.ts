/**
 * Merchant financial stats: CASH vs card (CARD / CREDIT_CARD) for completed-style orders.
 */
...
/** Order status rows that count toward "sales" in merchant reporting. */
export function isRevenueOrderStatus(status: unknown): boolean {
  const s = String(status ?? '').toUpperCase();
  return s === 'COMPLETED' || s === 'DELIVERED' || s === 'PAID';
}

/** Normalize payment method from order.payment.method or legacy paymentMethod. */
